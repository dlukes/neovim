...
describe('lua stdlib', function()
  ...
  describe('vim.parse_version', function()
    ...
    describe('in default mode', function()
      it('should handle a single number', function()
        eq({3}, funcs.luaeval('vim.parse_version("3")'))
      end)

      it('should handle arbitrarily many dotted numbers', function()
        eq({1, 42, 747, 5000}, funcs.luaeval('vim.parse_version("1.42.747.5000")'))
      end)

      it('should ignore trailing junk', function()
        eq({3}, funcs.luaeval('vim.parse_version("3,1415-beta3")'))
      end)

      it('should fail with leading junk', function()
        matches(err('v3'), pcall_err(funcs.luaeval, 'vim.parse_version("v3")'))
      end)

      it('should fail if the version string is empty', function()
        matches(err(''), pcall_err(funcs.luaeval, 'vim.parse_version("")'))
      end)
    end)

    describe('in strict (semver) mode', function()
      it('should handle a single number', function()
        eq({major=3}, funcs.luaeval('vim.parse_version("3", true)'))
      end)

      it('should handle up to three dotted numbers', function()
        eq({major=3, minor=2, patch=1}, funcs.luaeval('vim.parse_version("3.2.1", true)'))
      end)

      it('should fail with more than three dotted numbers', function()
        matches(err('1.2.3.4'), pcall_err(funcs.luaeval, 'vim.parse_version("1.2.3.4", true)'))
      end)

      it('should allow a pre-release label', function()
        eq({major=3, pre='alpha'}, funcs.luaeval('vim.parse_version("3-alpha", true)'))
      end)

      it('should allow a build label', function()
        eq({major=3, build='build'}, funcs.luaeval('vim.parse_version("3+build", true)'))
      end)

      it('should allow both a pre-release and a build label', function()
        eq({major=3, pre='alpha', build='build'}, funcs.luaeval('vim.parse_version("3-alpha+build", true)'))
      end)

      it('should not parse a pre-release label after a build label', function()
        eq({major=3, build='build-alpha'}, funcs.luaeval('vim.parse_version("3+build-alpha", true)'))
      end)

      it('should fail if the pre-release label does not match [0-9A-Za-z.-]', function()
        matches(err('3%-č'), pcall_err(funcs.luaeval, 'vim.parse_version("3-č", true)'))
      end)
      ...
      it('should fail with trailing junk', function()
        matches(err('3,1415%-beta3'), pcall_err(funcs.luaeval, 'vim.parse_version("3,1415-beta3", true)'))
      end)
    end)
  end)

  describe('vim.cmp_versions', function()
    describe('in default mode', function()
      it('should compare version numbers numerically', function()
        eq(-1, funcs.luaeval('vim.cmp_versions("1.2", "1.10")'))
      end)

      it('should work fine even if one number is shorter than the other', function()
        eq(1, funcs.luaeval('vim.cmp_versions("1.1", "1")'))
      end)

      it('should consider alternative spellings of the same version as equal', function()
        eq(0, funcs.luaeval('vim.cmp_versions("1", "1.0")'))
      end)
    end)

    describe('in strict (semver) mode', function()
      it('should try to recursively parse and compare pre-release labels', function()
        eq(-1, funcs.luaeval('vim.cmp_versions("1-1.2.3.4", "1-1.10.3.4", true)'))
      end)
      ...
      it('should ignore differences in build labels', function()
        eq(0, funcs.luaeval('vim.cmp_versions("1+build1", "1+build2", true)'))
      end)
    end)
  end)
end)
